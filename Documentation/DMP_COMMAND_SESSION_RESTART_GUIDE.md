...
## 📋 Handover prompt for the next session (paste this to start where this session left off)

```
Wir arbeiten an DMP_Command weiter. Letzte Sitzung endete mit v1.22.35 (lokal gepackt,
NICHT mehr in Studio getestet - ich musste Feierabend machen). Bitte zuerst den Session
Restart Guide lesen (aktuellster Recap-Abschnitt), dann:

1. Ich lade v1.22.35 jetzt in Studio und melde den App-Checker-Status (Fehler ja/nein,
   welche genau falls ja).
2. Falls fehlerfrei: bitte das neue Cockpit-"NEXT STEPS"-Panel inhaltlich mit mir testen
   (zeigt CoS-Leader-Checklisten-Aufgaben, Status-Punkte/-Texte, Idle-Meldung wenn kein
   DMP-Fall aktiv) UND prüfen, ob die blauen KPI-Change-LEDs (Critical/Warnings/Agents
   Active) noch unerwartet blinken (trat mit v1.22.33/34 auf, Ursache nie bestätigt).
3. Danach geht es weiter mit den offenen Next-Steps-Ausbaustufen (siehe Backlog
   Prioritaet 2 Punkt 1): Infrastructure/Content-Team-Checklisten (Site-Zugriff klaeren),
   Agent/Job fuer automatische Task-Occurrence-Erzeugung pro Fall, In-App-Vorschlagen/
   Bestaetigen, "DMP Stream Tasks"-Navigationsbereich, Agent-7-E-Mail-Automatisierung.
```

## ⚡ Latest session recap (2026-09-23, v1.22.33→v1.22.35: Next Steps Phase 1 implemented then debugged twice - Cockpit NEXT STEPS panel live on CoS Leader checklist data, session ended before final Studio confirmation)

- **v1.22.33 - initial implementation:** replaced the Cockpit's old static `varNextMilestones`-driven "NEXT STEPS" panel with a live view joining `DMP Command Checklist CoS Leader` with `DMP Command Checklist Overall Process` (by `Titel`). See the detailed recap below (originally written as "Latest", now superseded) for the full data-model-gap-analysis and architecture-decision narrative (CoS-Leader-only scope, checklist-Status-not-Task-Occurrences decision, `.msapr` schema refresh procedure) - none of that background changed, only the formula implementation was buggy and got fixed twice after this point.
- **v1.22.33 failed in Studio - 108 App Checker errors (~5 red badges), reported with a screenshot showing the exact error tree:** `AddColumns` reported "ungültige Argumente" and "Der Name ist ungültig - 'OverallProcessMatch'/'IsDoneCalc' wird nicht erkannt", cascading from `scrHome.OnVisible` (105 errors) into every gallery-template property referencing those columns. Root cause: **`AddColumns` column formulas can only see the ORIGINAL source table's own columns - never a column added by a nested/chained `AddColumns` passed as the `Source` argument, not even one level deep.** The original formula nested 3 levels of `AddColumns(AddColumns(AddColumns(...)))` inline.
- **v1.22.34 - first fix attempt:** rebuilt the same logic as 3 separate materialized stages (`ClearCollect(colCosLeaderStage1, ...)` → `ClearCollect(colCosLeaderStage2, AddColumns(colCosLeaderStage1, ...))` → `ClearCollect(colCosLeaderNextSteps, AddColumns(colCosLeaderStage2, ...))`), each stage a real, already-resolved collection before the next stage's `AddColumns` references it - this pattern is confirmed structurally sound and is the standard Power Fx workaround for this exact limitation. **The user reloaded v1.22.34 and the 5 red badges were still there, plus a newly-reported side effect: the blue KPI change-notification LEDs (Critical/Warnings/Agents Active) started blinking unexpectedly** (cause not confirmed - hypothesis is it's a side effect of the app being in a compile-error state, not something the new timer itself causes, since `tmrCosLeaderNextStepsRefresh` never touches `varBlinkPhase`).
- **v1.22.35 - second, independently-found fix (session's last action, NOT yet tested):** re-examined the formula and found a SECOND, separate bug that the v1.22.34 fix didn't touch: **`Split(Text, Separator)` returns a table with column name `Value` - the formula used the wrong name `Result`** (`Filter(Split(OverallProcessMatch.PredecessorTaskIds, ";"), LookUp(...Title = Trim(Result))...)` - confirmed wrong against the official Power Fx documentation, fetched live this session). This is very likely why the 5 badges persisted after v1.22.34's otherwise-correct `AddColumns` restructure - the compile would have hit this second, independent error right after the first was fixed. Corrected `Trim(Result)` → `Trim(Value)` in both `OnVisible` and the timer.
- **Real, confirmed bug fixed in Configuration (Lists), unrelated to Next Steps:** the user explicitly confirmed `DMP Command External Domains` has **no `Active` column by design** (Agent 1 fully rewrites this list from scratch on every Emergency Report extraction, so there is no persistent "active" flag to filter on). The v1.22.30-era count formula (`Filter('DMP Command External Domains', Active.Value = "Yes")`) was therefore a genuine, long-standing bug that had gone unnoticed until this session's `.msapr` schema refresh made Studio's App Checker actually validate the field reference against the live schema (previously masked by a stale schema cache) - it showed up as 3 separate `scrConfiguration` errors in the user's screenshot. Fixed to a plain `Total rows` count (matching the pattern already used for every other list without an `Active` column); Operations Manual (§3.5 table + panel description) and the in-app Operational Manual text corrected to match.
- **Encoding discipline reinforced (again):** all fixes this round were applied via the `edit` tool or via `[System.IO.File]::ReadAllText/WriteAllText` with explicit `UTF8Encoding`, per the lesson from earlier in this same session (a prior `Get-Content`/`WriteAllText`-without-explicit-encoding script had silently corrupted `·`/`★` characters app-wide) - verified via a post-edit `Â·`/`â˜…` grep scan (0 hits) each time.
- **Deployment hygiene (v1.22.35):** app-wide control-name uniqueness (869 names, 0 duplicates), literal `": "` regex scan (0 hits), full pack→unpack round-trip diff = 0 on all 5 changed files (`scrHome`, `scrConfiguration`, `scrOperationalBoard`, `scrReleaseNotes`, `App`). Documentation (Release Notes in-app + `.md`, Operations Manual `.md` + in-app, Backlog, this guide) all updated per the 1:1-parity rule and synced to the OneDrive copy.
- **Versioning:** bumped `v1.22.33` → `v1.22.34` → `v1.22.35` across this debugging arc (one new version per fix attempt, per the "always bump patch" rule, even though none of the three has been confirmed loading yet). `PowerApp_Version.txt` reflects `v1.22.35`. Rule 9b backup deliberately still at `DMP_COMMAND_v1.22.32.msapp` (the last version the user actually confirmed loads cleanly) - do not rotate it until the user confirms `v1.22.35` (or whatever superseding version) loads without errors.
- **Session ended here at the user's request ("möchte jetzt meinen Tag beenden") before `v1.22.35` could be loaded/tested.** This is the single most important thing for the next session to resolve first - do not assume the fixes work; ask for the current App Checker state as the very first step, per the handover prompt above.

## ⚡ Prior session recap (2026-09-23, v1.22.33: Next Steps Phase 1 implemented - Cockpit NEXT STEPS panel now live on CoS Leader checklist data, not yet Studio-tested)
