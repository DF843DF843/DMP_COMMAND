...
- **Before starting new work or any deployment package, check whether related, already-scoped backlog items can be bundled into the same change**, to reduce the number of separate deployment cycles — but only for items that need no further human decision; anything requiring a judgment call is proposed to the human first, never assumed. Version bumps, pack operations, imports, and publishes must happen only after this backlog-bundling check unless the human explicitly requests an immediate single hotfix.
- **After every deployment, hand over deliberately and recommend a fresh session.** The AI summarizes what was deployed, versions before/after, validation performed, required live tests, and remaining backlog items, then reminds the human to continue in a new session to reduce token/credit use. Future sessions restart from `Documentation/DMP_COMMAND_SESSION_RESTART_GUIDE.md`.
