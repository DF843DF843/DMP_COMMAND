...
2. **Configuration-Schema für 5 Modi:** ✅ **Korrigiert am 2026-09-22 (Variante B, symmetrisch):** Bestehende 4 Modus-Spalten in `DMP Command Configuration` werden um **4 weitere Spalten** ergänzt → 8 Werte-Spalten total (`Value - PROD (Pre-Default)`, `Value - PROD (Post-Default)`, `Value - SIMU (Pre-Default)`, `Value - SIMU (Post-Default)`, zusätzlich zu den bestehenden 4 `Value - PROD/SIMU (NODMP/DMP)`-Spalten; "Normal" entspricht weiterhin dem bisherigen `NODMP`-Wert). Betrifft nur Parameter, die Agent 5 (bzw. die App-Anzeige) tatsächlich mode-abhängig nutzt, nicht die gesamte Liste.
2b. **Kein dritter "TEST"-Umgebungswert nötig (Klarstellung 2026-09-22):** Der Nutzer wollte ursprünglich zusätzlich zwischen `LIVE`/`SIMU`/`TEST` unterscheiden. Klargestellt: `PROD` und `SIMU` sind beides Betriebsmodi **innerhalb derselben echten (Live-)Umgebung** (SIMU = Fire-Drill-Prozessübung im selben System, keine technische Trennung). „TEST" entspricht dagegen der bereits strukturell komplett getrennten Power-Platform-Umgebung `DBG Team Productivity (Dev)`/`(UAT)` (eigene SharePoint-Listen, eigenes Postfach, eigene Flow-Instanzen) — das ist bereits vorhanden und braucht keine dritte Konfigurationswert-Spalte. `DMP Command Configuration` bleibt bei genau 2 Umgebungen (PROD/SIMU) × 4 Modi = 8 Spalten.
